/************************************************************************/
/* CCITT G.72x Common Routines: G2XXlt.c                V2.00  04/15/95 */
    ...
    int     size)       /* table size of short integers */
{
    ...
    mant = (short) (((dqm << 7) >> exp) & 0x7F);    /* Fractional portion. */
    dl = (exp << 7) + mant;
    ...
    int     y)                  /* Step size multiplier                 */
{
    ...
    short   dqt;     
    short   dq;                 /* Reconstructed difference signal samp */
  
    dql = dqln + (y >> 2);      /* ADDA */
  
    if (dql < 0) {
        return ((sign) ? -(int)0x8000 : 0);
    } else {                    /* ANTILOG */
        dex = (dql >> 7) & 15;
        dqt = 128 + (dql & 127);
        dq = (dqt << 7) >> (14 - dex);
