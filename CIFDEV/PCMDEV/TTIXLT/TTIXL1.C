/************************************************************************/
/* TTI Powerline II PCM Translator: TTIXl1.c            V2.00  06/03/92 */
...
/************************************************************************/
#include "ttixl1.tab"                   /* cs:Step tables               */
...
/************************************************************************/
LPITCB FAR PASCAL PL2A04toIni (LPITCB lpITC, LPVOID lpRsv001)
{
    lmemset (lpITC, 0, sizeof (*lpITC));
    return (lpITC);                     /* Return Init/Term/Cont block  */
}

WORD FAR PASCAL PL2A04toCmp (_segment _sBufSeg, BYTBASPTR pcSrcBuf, 
...
/************************************************************************/
WORD FAR PASCAL PL2A04toG16 (_segment _sBufSeg, BYTBASPTR pcSrcBuf,
...
/************************************************************************/
WORD FAR PASCAL PL2G16toA04 (_segment _sBufSeg, GENBASPTR psSrcBuf,
...
wastrt:     mov     ax, _sBufSeg
            ...
            //***********************************************************
fstnib:     lodsw                       // AX = SI++ -> next waveform value
            ...
            shr     ax, 1               // Position first sample
...
tstss1:     dec     usSmpCnt            // Done with all samples?
            ...
            //***********************************************************
secnib:     lodsw                       // AX = SI++ -> next waveform value
            sub     ax, cx              // AX = delta for next wave value
            cwd                         // DX = Sign of AX (extended)
            SHR4    (bx)                // Adjust for table word lookup
            idiv    T_ StpTab[bx]       // AX = delta/stepsize + rem DX
...
/************************************************************************/
WORD FAR PASCAL PL2A04toM32 (_segment _sBufSeg, BYTBASPTR pcSrcBuf,
...
/************************************************************************/
WORD FAR PASCAL PL2SiltoA04 (BYTE FAR *lpcDstBuf, WORD usBufSiz)
{
    WORD    usi = usBufSiz;
