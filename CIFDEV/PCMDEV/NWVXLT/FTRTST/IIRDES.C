...
#include "iirdes.h"                     /* IIR filter design defs       */
...
WORD FAR PASCAL EffIIROpt (WORD usFtrTyp, WORD usSecCnt, WORD usSecLen,
                DWORD ulStpFrq, float flPasFac, float flStpAtt, 
                DWORD ulSmpFrq, BOOL bfFstFlg, CVSFTR FAR *lpCVSFtr)
{
    ...
    /********************************************************************/
    if (bfFstFlg) {
        for (usi = 0; usi < lpCVSFtr->usSec * lpCVSFtr->fbZer.usLen; usi++) 
            lpCVSFtr->fbZer.lfCoe[usi] = (LNGFRA) (ONE * lpCVSFtr->fbZer.flCoe[usi]);
        for (usi = 0; usi < lpCVSFtr->usSec * lpCVSFtr->fbPol.usLen; usi++) 
