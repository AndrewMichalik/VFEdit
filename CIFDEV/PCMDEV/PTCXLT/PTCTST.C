/************************************************************************/
/* PTCTst: Conversion Tester                            V2.00  08/15/95 */
...
/*                      External References                             */
/************************************************************************/
ALOITCPRC   DefAloITC = PTCC01toAlo;
CMPITCPRC   DefCmpIni = PTCC01toCmp;
PCMTOGPRC   DefPCMToG = PTCC01toG16;
GTOPCMPRC   DefGToPCM = PTCG16toC01;
PCMTOMPRC   DefPCMToO = PTCC01toM32;

char    MsgUseIni[] =  
"\n\
  Voice Information Systems (1-800-234-VISI) Perception Conversion Utility\n\
           v0.10a.0 (C) Copyright Andrew J. Michalik 1987-1993\n\n";
...
char **envp;
{
    char    szSrcFil[FIOMAXNAM];
    char    szDstFil[FIOMAXNAM];
    ...
    if (strcmp (*++argv, "-h") == 0) DspHlpEnd ();
    if (argc <= 2) UseErrEnd (NULL);
    argc = argc - 3;                    /* Indicate name & 2 args used  */

    /********************************************************************/
    /* Get file names                                                   */
    /********************************************************************/
    strncpy (szSrcFil, *argv++, FIOMAXNAM);
    strncpy (szDstFil, *argv++, FIOMAXNAM);

    /********************************************************************/
    /********************************************************************/
//    TstC01toG ("jimtst.v01", "jimtst.v16");
    TstGtoC01 (szSrcFil, szDstFil);
//    TstC01toG (szSrcFil, szDstFil);
    exit (0);
