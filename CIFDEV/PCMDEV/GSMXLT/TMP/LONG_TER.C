...
unsigned long umul_table[ 513 ][ 256 ];

void init_umul_table()
{
		long 	i, j;
		long 	n;
		unsigned long	 * p = &umul_table[0][0];
...
# define umul(x9, x15)  \
		((long)(umul_table[x9][x15 & 0x0FF] + (umul_table[x9][ x15 >> 8 ] << 8)))
...
/* Microsoft's ever-so-carefully-debugged Visual C++ 1.52 loses it
   ...
   expansion */		
/* JMS -- Works in 1.50.  Guess we'll skip 1.52.  */
