/************************************************************************/
/* Signal Processing IIR Filter Design                  V2.00  07/03/93 */
...
/************************************************************************/
/*                  IIR Low Pass filter allocation                      */
/************************************************************************/
WORD FAR PASCAL EffILPAlo (WORD usFtrTyp, WORD usSecCnt, WORD usSecLen,
                ...
                DWORD ulSmpFrq, BOOL bfFstFlg, CVSFTR FAR *lpCVSFtr)
{
    ...
    /********************************************************************/
    if (!usSecLen || ((usSecCnt * usSecLen) > CVSFCBMAX)) return ((WORD) -1);
    if ((ulStpFrq >= ulSmpFrq / 2) || (flGrdFac > 1)) return ((WORD) -1);
    ...
    spiird (&lFtrTyp, &lPasTyp, &lSecCnt, &lSecLen, &fFrq001, &fFrq002, 
        &fFrq003, &fFrq004, &fStpAtt, lpCVSFtr->fbZer.flCoe, 
        lpCVSFtr->fbPol.flCoe, &lErrCod);
    if (lErrCod) return ((WORD) lErrCod);

    /********************************************************************/
    /********************************************************************/
    lpCVSFtr->usTyp = usFtrTyp;
    lpCVSFtr->usSec = usSecCnt;    
    lpCVSFtr->fbZer.usLen = usSecLen;
    lpCVSFtr->fbPol.usLen = usSecLen - 1;

    /********************************************************************/
    /* Adjust coefficients for long fraction math (if requested)        */
    /********************************************************************/
    if (bfFstFlg) {
        for (usi = 0; usi < lpCVSFtr->fbZer.usLen; usi++) 
            lpCVSFtr->fbZer.lfCoe[usi] = (LNGFRA) (ONE * lpCVSFtr->fbZer.flCoe[usi]);
        for (usi = 0; usi < lpCVSFtr->fbPol.usLen; usi++) 
            lpCVSFtr->fbPol.lfCoe[usi] = (LNGFRA) (ONE * lpCVSFtr->fbPol.flCoe[usi]);
    }
...
main ()
{
    ...
    /********************************************************************/
    /********************************************************************/
    printf ("CHII Low Pass: %ld\tGrd: %f\tSec: %d\tLen: %d\tRej: %f", 
        ulStpFrq, flGrdFac, usSecCnt, usSecLen, flStpAtt);
